...
client.on("guildMemberAdd", (member) => {
    ...
    guild.fetchInvites().then(invites => {
       ...
       client.channels.cache.get("810794364329656321").send(`${member} Adlı Kişi Sunucuya Katıldı ! **Davet Eden:** ${invite.inviter.tag} (**${total + bonus}** Davet!) <a:onayl:787289361414225950>`)
...
client.on("guildMemberRemove", (member) => {
    ...
    db.add(`invites.${data.inviter}.leave`, 1);
        let user = client.users.cache.get(data.inviter)
     	client.channels.cache.get("810794364329656321").send(`${member.user.tag} Sunucudan Ayrıldı ! **Davet Eden:** ${user.tag} <a:onaysz:787289292137431040>`)
     }
});
